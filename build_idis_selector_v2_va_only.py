...
def parse_args():
    p = argparse.ArgumentParser(
        description="Build IDIS Product Selector HTML. Use --va-only to generate a VA Selector only HTML."
    )
    p.add_argument(
        "--va-only",
        action="store_true",
        help="Generate an output HTML that only exposes the VA Selector screen."
    )
    p.add_argument(
        "--output",
        "-o",
        default=None,
        help="Output HTML path. Default: IDIS_Product_Selector_Final.html or IDIS_Product_Selector_VA_Only.html with --va-only."
    )
    return p.parse_args()

ARGS = parse_args()
VA_ONLY = ARGS.va_only
OUT = Path(ARGS.output) if ARGS.output else Path(
    "IDIS_Product_Selector_VA_Only.html" if VA_ONLY else "IDIS_Product_Selector_Final.html"
)
...
def apply_va_only_mode(html: str) -> str:
    """Hide every screen except VA Selector in the generated HTML.

    The original HTML/JS is intentionally kept intact because many existing
    functions are initialized globally. Removing Camera/VMS/Recorder DOM nodes
    can create null-reference errors. This mode only changes the final UI:
      - VA Selector is forced to be the only visible screen.
      - Non-VA navigation buttons are hidden.
      - goScreen() is overridden so the page always stays on VA.
    """
    va_css = """

<!-- VA ONLY MODE -->
<style id="vaOnlyModeCss">
  #scrCam,
  #scrVms,
  #scrRec,
  #scrIss,
  ...
  #dtlOv,
  #cmpOv,
  #rCmpOv { display:none !important; }

  #scrVa { display:block !important; }

  #navCam,
  #navVms,
  #navRec,
  #navIss,
  #navExpl { display:none !important; }
...
<script id="vaOnlyModeScript">
(function(){
  function activateVaOnly(){
    document.body.classList.add('va-only-mode');

    ['Cam','Vms','Rec','Iss','Expl'].forEach(function(name){
      var el=document.getElementById('scr'+name);
      if(el) el.className='screen';
    });

    var va=document.getElementById('scrVa');
    if(va) va.className='screen on';

    ['Cam','Vms','Rec','Iss','Expl'].forEach(function(name){
      var nav=document.getElementById('nav'+name);
      if(nav) nav.className='nav-btn';
    });

    var navVa=document.getElementById('navVa');
    if(navVa){
      navVa.className='nav-btn nav-on';
      navVa.textContent='VA Selector';
      navVa.onclick=function(){ return false; };
    }
  }

  activateVaOnly();

  // Prevent accidental navigation to hidden screens from console or old handlers.
  window.goScreen=function(){ activateVaOnly(); };

  document.addEventListener('DOMContentLoaded', activateVaOnly);
  setTimeout(activateVaOnly, 0);
})();
</script>
"""

    if "</head>" not in html:
        fail("</head> not found while applying VA-only mode")
    if "</body>" not in html:
        fail("</body> not found while applying VA-only mode")

    html = html.replace("</head>", va_css + "\n</head>", 1)
    html = html.replace("</body>", va_js + "\n</body>", 1)
    return html
...
# ─────────────────────────────────────────────────────────────
print("=" * 62)
print("  IDIS Product Selector Builder  v2" + ("  [VA ONLY]" if VA_ONLY else ""))
print("=" * 62)
...
# ─────────────────────────────────────────────────────────────
# 10b. VA Selector 단독 모드 적용
# ─────────────────────────────────────────────────────────────
if VA_ONLY:
    print("\n[10b] Applying VA Selector only mode ...")
    src = apply_va_only_mode(src)
    ok("VA-only mode enabled: only VA Selector is exposed in the final HTML")
...
{'=' * 62}
  Output : {OUT}
  Mode   : {"VA Selector only" if VA_ONLY else "Full Product Selector"}
